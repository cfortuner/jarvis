...
def load_intent_phrase_map_from_modules(dir_name: str) -> dict:
    ...
    for file_name in file_list:
        ...
        if os.path.isdir(full_path) and \
            ...
            phrase_map.update(load_intent_phrase_map_from_modules(rel_path))
        elif full_path.endswith(const.INTENT_FILE_SUFFIX) and file_name != "__init__.py":
            ...
            for _, t_value in module.__dict__.items():
                try:
                    if issubclass(t_value, IntentParser):
...
def find_matching_intents(
    ...
    for phrase_matcher, action_type in phrase_map.items():
        ...
        if status:
            intents.append((action_type, params))
    return intents
